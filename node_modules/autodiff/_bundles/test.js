...
exports.__esModule = true;
var autodiffTest = __webpack_require__(16);
var seriesTest = __webpack_require__(17);
var series2DTest = __webpack_require__(18);
var parserTest = __webpack_require__(19);
var tokenizerTest = __webpack_require__(20);
void autodiffTest;
void seriesTest;
void series2DTest;
void parserTest;
...
var autodiff_context_1 = __webpack_require__(0);
var series_1 = __webpack_require__(2);
function expectDerivatives(series, expected) {
    var derivatives = series_1.toValueAndDerivatives(series);
...
describe('series', function () {
    beforeEach(function () {
        autodiff_context_1.defaultContext.setNumberOfDerivativesToCompute(3);
    });
    it('negative', function () {
        var x = series_1.variableEvaluatedAtPoint(2);
        expectDerivatives(series_1.negative(x), [-2, -1, 0, 0]);
        var value = series_1.add(x, x);
    ...
    it('multiply', function () {
        var x = series_1.variableEvaluatedAtPoint(2);
        var value = series_1.multiply(x, 132);
        expectDerivatives(value, [264, 132, 0, 0]);
        value = series_1.multiply(x, x);
        expectDerivatives(value, [4, 4, 2, 0]);
        x = series_1.variableEvaluatedAtPoint(-1);
        value = series_1.multiply(series_1.multiply(series_1.multiply(x, x), x), 4);
        expectDerivatives(value, [-4, 12, -24, 24]);
    });
    it('divide', function () {
        var x = series_1.variableEvaluatedAtPoint(6);
        var value = series_1.divide(x, 3);
        expectDerivatives(value, [2, 1 / 3, 0, 0]);
        x = series_1.variableEvaluatedAtPoint(2);
        var twoX = series_1.add(x, 2);
        value = series_1.divide(twoX, x);
        expectDerivatives(value, [2, -.5, .5, -.75]);
    });
    it('sqrt', function () {
        var x = series_1.variableEvaluatedAtPoint(16);
        var value = series_1.sqrt(x);
        expectDerivatives(value, [4, 0.125, -0.00390625, 0.0003662109375]);
        var pt = 16;
        x = series_1.variableEvaluatedAtPoint(pt);
        value = series_1.sqrt(series_1.add(x, 1));
    ...
    it('exp for x > 0, x = 0 and x < 0', function () {
        for (var _i = 0, _a = [2, 0, -2]; _i < _a.length; _i++) {
            var pt = _a[_i];
            var x = series_1.variableEvaluatedAtPoint(pt);
            var value = series_1.exp(series_1.multiply(x, x));
            expectDerivatives(value, [
                Math.exp(pt * pt),
                2 * pt * Math.exp(pt * pt),
    ...
    it('log of x > 1, x < 1 and x < 0', function () {
        for (var _i = 0, _a = [Math.E, .5, -1]; _i < _a.length; _i++) {
            var pt = _a[_i];
            var x = series_1.variableEvaluatedAtPoint(pt);
            var value = series_1.log(series_1.multiply(x, x));
            expectDerivatives(value, [
                Math.log(pt * pt),
                2 / pt,
                -2 / (pt * pt),
    ...
    it('pow(a, b) where a >= 0', function () {
        for (var _i = 0, _a = [0, .5, 2]; _i < _a.length; _i++) {
            var pt = _a[_i];
            for (var _b = 0, _c = [-2, -.5, 0, .5, 2]; _b < _c.length; _b++) {
                var b = _c[_b];
                var x = series_1.variableEvaluatedAtPoint(pt);
                var value = series_1.pow(x, b);
                expectDerivatives(value, [
                    Math.pow(pt, b),
                    b * Math.pow(pt, (b - 1)),
                    b * (b - 1) * Math.pow(pt, (b - 2)),
                    b * (b - 1) * (b - 2) * Math.pow(pt, (b - 3)),
                ]);
            }
        }
    });
    ...
    it('pow(a, b) where both a and b are series', function () {
        var pt = -3;
        var x = series_1.variableEvaluatedAtPoint(pt);
        var value = series_1.pow(x, x);
        expectDerivatives(value, [
            Math.pow(pt, pt),
            Math.pow(pt, pt) * (Math.log(pt) + 1),
            Math.pow(pt, pt) * (1 / pt + Math.pow(Math.log(pt) + 1, 2)),
            Math.pow(pt, pt - 2) * (pt * pt +
                pt * pt * Math.pow(Math.log(pt), 3) +
                3 * pt * pt * Math.pow(Math.log(pt), 2) +
                3 * pt +
    ...
    it('sin', function () {
        var pt = 2;
        var x = series_1.variableEvaluatedAtPoint(pt);
        var value = series_1.sin(series_1.add(x, 1));
        expectDerivatives(value, [
            Math.sin(pt + 1),
            Math.cos(pt + 1),
            -Math.sin(pt + 1),
    ...
    it('cos', function () {
        var pt = 2;
        var x = series_1.variableEvaluatedAtPoint(pt);
        var value = series_1.cos(series_1.add(x, 1));
        expectDerivatives(value, [
            Math.cos(pt + 1),
            -Math.sin(pt + 1),
            -Math.cos(pt + 1),
    ...
    it('tan', function () {
        var pt = 2;
        var x = series_1.variableEvaluatedAtPoint(pt);
        var value = series_1.tan(series_1.add(x, 1));
        var t = Math.tan(pt + 1);
        var s = 1 / Math.cos(pt + 1);
        expectDerivatives(value, [
            t,
            s * s,
            2 * t * s * s,
...
var series2d_1 = __webpack_require__(4);
function expectDerivatives(series, expected) {
    var derivatives = series2d_1.toValueAndDerivatives(series);
    expect(derivatives.length).toBe(expected.length);
    var areEqual = true;
    for (var i = 0; i < derivatives.length; i++) {
        if (Math.abs(derivatives[i] - expected[i]) >= .001) {
...
describe('series2D', function () {
    beforeEach(function () {
        autodiff_context_1.defaultContext.setNumberOfDerivativesToCompute(2);
    });
    it('convolve', function () {
        var a = new series2d_1.Series2D();
        a.coefficients = [
            1, 2, 3,
            4, 5, 6,
            7, 8, 9
        ];
        var b = new series2d_1.Series2D();
        b.coefficients = [
            10, 11, 12,
            13, 14, 15,
            16, 17, 18
        ];
        expect(series2d_1.convolve(a, b, 1, 1)).toBe(1 * 10);
        expect(series2d_1.convolve(a, b, 1, 2)).toBe(1 * 13 + 4 * 10);
        expect(series2d_1.convolve(a, b, 2, 1)).toBe(1 * 11 + 2 * 10);
        expect(series2d_1.convolve(a, b, 1, 3)).toBe(1 * 16 + 4 * 13 + 7 * 10);
        expect(series2d_1.convolve(a, b, 3, 1)).toBe(1 * 12 + 2 * 11 + 3 * 10);
        expect(series2d_1.convolve(a, b, 2, 2)).toBe(1 * 14 + 2 * 13 + 4 * 11 + 5 * 10);
        expect(series2d_1.convolve(a, b, 2, 3)).toBe(1 * 17 + 2 * 16 +
            4 * 14 + 5 * 13 +
            7 * 11 + 8 * 10);
        expect(series2d_1.convolve(a, b, 3, 2)).toBe(1 * 15 + 2 * 14 + 3 * 13 +
            4 * 12 + 5 * 11 + 6 * 10);
        expect(series2d_1.convolve(a, b, 3, 3)).toBe(1 * 18 + 2 * 17 + 3 * 16 +
            4 * 15 + 5 * 14 + 6 * 13 +
            7 * 12 + 8 * 11 + 9 * 10);
    });
    it('dConvolve', function () {
        var a = new series2d_1.Series2D();
        a.coefficients = [
            1, 2, 3,
            4, 5, 6,
        ...
        var index = [2, 2];
        var k_p = 2;
        expect(series2d_1.dConvolve(a, b, index[0], index[1])).toBe((1 * (2 * 17 + 5 * 14 + 8 * 11) +
            2 * (3 * 16 + 6 * 13)) / k_p);
        index = [1, 1];
        k_p = 1;
        expect(series2d_1.dConvolve(a, b, index[0], index[1])).toBe((1 * (2 * 13)) / k_p);
        index = [0, 2];
        k_p = 2;
        expect(series2d_1.dConvolve(a, b, index[0], index[1])).toBe((1 * (4 * 13)) / k_p);
    });
    it('creating constants and values', function () {
        var x = series2d_1.xEvaluatedAtPoint(2);
        expectDerivatives(x, [
            2, 1, 0,
            0, 0, 0,
            0, 0, 0,
        ]);
        var y = series2d_1.yEvaluatedAtPoint(2);
        expectDerivatives(y, [
            2, 0, 0,
            1, 0, 0,
            0, 0, 0,
        ]);
        var c = series2d_1.constantValue(2);
        expectDerivatives(c, [
            2, 0, 0,
            0, 0, 0,
            0, 0, 0,
    ...
    it('negative', function () {
        var x = series2d_1.xEvaluatedAtPoint(2);
        var negX = series2d_1.negative(x);
        ...
        var y = series2d_1.yEvaluatedAtPoint(2);
        var negY = series2d_1.negative(y);
        expectDerivatives(negY, [
            -2, 0, 0,
            -1, 0, 0,
            0, 0, 0,
        ]);
        var f = series2d_1.add(negX, negY);
        expectDerivatives(f, [
            -4, -1, 0,
            -1, 0, 0,
            0, 0, 0,
    ...
    it('add', function () {
        var x = series2d_1.xEvaluatedAtPoint(2);
        var y = series2d_1.yEvaluatedAtPoint(2);
        var value = series2d_1.add(series2d_1.add(x, x), series2d_1.add(y, y));
        expectDerivatives(value, [
            8, 2, 0,
            2, 0, 0,
            0, 0, 0,
    ...
    it('multiply', function () {
        var x = 3;
        var y = 2;
        var xVal = series2d_1.xEvaluatedAtPoint(x);
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var value = series2d_1.multiply(series2d_1.multiply(xVal, xVal), series2d_1.multiply(yVal, yVal));
        expectDerivatives(value, [
        ...
        for (var _i = 0, _a = [-1, 0, 1]; _i < _a.length; _i++) {
            ...
            for (var _b = 0, _c = [-1, 0, 1]; _b < _c.length; _b++) {
                var y_1 = _c[_b];
                xVal = series2d_1.xEvaluatedAtPoint(x_1);
                yVal = series2d_1.yEvaluatedAtPoint(y_1);
                value = series2d_1.multiply(xVal, yVal);
                expectDerivatives(value, [
                    x_1 * y_1, y_1, 0,
                    x_1, 1, 0,
                    0, 0, 0,
    ...
    it('divide', function () {
        ...
        var xVal = series2d_1.xEvaluatedAtPoint(x);
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var value = series2d_1.divide(xVal, yVal);
        expectDerivatives(value, [
            x / y, 1 / y, 0,
            -x / (y * y), -1 / (y * y), 0,
            2 * x / (y * y * y), 2 / (y * y * y), 0
    ...
    function testExy(x, y) {
        it("exp for x = " + x + ", y = " + y, function () {
            var xVal = series2d_1.xEvaluatedAtPoint(x);
            var yVal = series2d_1.yEvaluatedAtPoint(y);
            var xyVal = series2d_1.multiply(xVal, yVal);
            expectDerivatives(xyVal, [
                x * y, y, 0,
                x, 1, 0,
                0, 0, 0
            ]);
            var value = series2d_1.exp(xyVal);
            var exy = Math.exp(x * y);
            expectDerivatives(value, [
                exy, y * exy, y * y * exy,
    ...
    for (var _i = 0, _a = [-2, 0, 2]; _i < _a.length; _i++) {
        var x = _a[_i];
        for (var _b = 0, _c = [-2, 0, 2]; _b < _c.length; _b++) {
            var y = _c[_b];
            testExy(x, y);
        }
    }
    it('log of x > 1, x < 1 and x < 0', function () {
        for (var _i = 0, _a = [Math.E, .5, -1]; _i < _a.length; _i++) {
            var x = _a[_i];
            for (var _b = 0, _c = [Math.E, .5, -1]; _b < _c.length; _b++) {
                var y = _c[_b];
                var xVal = series2d_1.xEvaluatedAtPoint(x);
                var yVal = series2d_1.yEvaluatedAtPoint(y);
                var xyVal = series2d_1.multiply(xVal, yVal);
                var logVal = series2d_1.log(xyVal);
    ...
    it('sqrt', function () {
        ...
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var xyVal = series2d_1.multiply(xVal, yVal);
        var sqrtVal = series2d_1.sqrt(xyVal);
    ...
    it('pow(a, b) where a >= 0', function () {
        for (var _i = 0, _a = [0, .5, 2]; _i < _a.length; _i++) {
            ...
            for (var _b = 0, _c = [-2, -.5, 0, .5, 2]; _b < _c.length; _b++) {
                ...
                var yVal = series2d_1.yEvaluatedAtPoint(y);
                var xyVal = series2d_1.multiply(xVal, yVal);
                var powVal = series2d_1.pow(xyVal, b);
                var xyb = Math.pow(x * y, b);
                expectDerivatives(powVal, [
                    xyb, b * xyb / x, (b - 1) * b * xyb / (x * x),
                    b * xyb / y, b * b * Math.pow(x * y, b - 1), (b - 1) * b * b * y * Math.pow(x * y, b - 2),
                    (b - 1) * b * xyb / (y * y), (b - 1) * b * b * x * Math.pow(x * y, b - 2), Math.pow((b - 1), 2) * Math.pow(b, 2) * xyb / (x * x * y * y)
                ]);
            }
        }
    });
    it('pow(a, b) where a < 0', function () {
        var pt = -1;
        var x = series2d_1.xEvaluatedAtPoint(pt);
        var y = series2d_1.yEvaluatedAtPoint(pt);
        var value = series2d_1.pow(x, y);
        var coefficients = value.coefficients;
    ...
    it('sin', function () {
        var x = 2;
        var y = 3;
        var sinxy = Math.sin(x * y);
        var cosxy = Math.cos(x * y);
        var xVal = series2d_1.xEvaluatedAtPoint(x);
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var xyVal = series2d_1.multiply(xVal, yVal);
        var sinVal = series2d_1.sin(xyVal);
    ...
    it('cos', function () {
        ...
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var xyVal = series2d_1.multiply(xVal, yVal);
        var sinVal = series2d_1.cos(xyVal);
        expectDerivatives(sinVal, [
    ...
    it('tan', function () {
        ...
        var y = 3;
        var cosxy = Math.cos(x * y);
        var cosxy2 = cosxy * cosxy;
        var secxy2 = 1 / cosxy2;
        var secxy4 = secxy2 * secxy2;
        var xVal = series2d_1.xEvaluatedAtPoint(x);
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var xyVal = series2d_1.multiply(xVal, yVal);
        var tanVal = series2d_1.tan(xyVal);
        expectDerivatives(tanVal, [
            Math.tan(x * y), y / cosxy2, 2 * y * y * Math.tan(x * y) / cosxy2,
            x / cosxy2,
            1 / cosxy2 + 2 * x * y * Math.tan(x * y) / cosxy2,
            2 * x * y * y * secxy4 + 4 * x * y * y * Math.pow(Math.tan(x * y), 2) * secxy2 + 4 * y * Math.tan(x * y) * secxy2,
    ...
    it('asin', function () {
        ...
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var xyVal = series2d_1.multiply(xVal, yVal);
        var asinVal = series2d_1.asin(xyVal);
    ...
    it('acos', function () {
        ...
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var xyVal = series2d_1.multiply(xVal, yVal);
        var asinVal = series2d_1.acos(xyVal);
        expectDerivatives(asinVal, [
            Math.acos(x * y),
    ...
    it('atan', function () {
        ...
        var yVal = series2d_1.yEvaluatedAtPoint(y);
        var xyVal = series2d_1.multiply(xVal, yVal);
        var atanVal = series2d_1.atan(xyVal);
        expectDerivatives(atanVal, [
            ...
            -4 * x * y * (Math.pow(x, 4) * Math.pow(y, 4) - 8 * x * x * y * y + 3) / Math.pow(x * x * y * y + 1, 4)
        ]);
    });
});
...
var expression_parser_1 = __webpack_require__(3);
var error_1 = __webpack_require__(1);
function checkParse(input, expected) {
    var res = expression_parser_1.parseExpression(input);
    expect(res.userReadableError).toBe(null);
    expect(expression_parser_1.toDebugString(res.expression)).toEqual(expected);
}
function checkParseFails(input, expected) {
    var res = expression_parser_1.parseExpression(input);
...
describe('expression parser', function () {
    ...
    it('should treat math operations with appropriate precedence', function () {
        checkParse("a + b * c", "[[a] + [[b] * [c]]]");
        checkParse("a - b / c", "[[a] - [[b] / [c]]]");
        checkParse("a * b ^ c", "[[a] * [[b] ^ [c]]]");
    });
    it('should give errors for invalid expressions', function () {
        checkParseFails("x + (y +", error_1.ErrorMsg.operatorMissingRightOperand('+'));
        checkParseFails("x + (y", error_1.ErrorMsg.missingRightParen);
        checkParseFails("x + (", error_1.ErrorMsg.genericFailure);
        checkParseFails("3~", error_1.ErrorMsg.invalidCharacter('~'));
        checkParseFails("x!", error_1.ErrorMsg.invalidCharacter('!'));
    });
});
...
var tokenizer_1 = __webpack_require__(5);
var error_1 = __webpack_require__(1);
function checkTokens(input, expected) {
    var tokens = tokenizer_1.tokenize(input).map(function (t) { return t.value; });
    expect(tokens).toEqual(expected);
}
function checkError(input, expected) {
    var didThrow = false;
    try {
        tokenizer_1.tokenize(input);
    }
    catch (e) {
        didThrow = true;
        expect(error_1.isParserError(e)).toBe(true);
        if (error_1.isParserError(e)) {
...
describe('tokenizer', function () {
    ...
    it('should insert multiplications when necessary', function () {
        checkTokens('a b', ['a', '*', 'b']);
        checkTokens('(a) (b)', ['(', 'a', ')', '*', '(', 'b', ')']);
        checkTokens('3 4', ['3', '*', '4']);
        checkTokens('3. .4', ['3.', '*', '.4']);
    ...
    it('should give errors for invalid tokens', function () {
        checkError('c.d', error_1.ErrorMsg.invalidCharacter('.'));
        checkError('[a]', error_1.ErrorMsg.invalidCharacter('['));
        checkError('3~', error_1.ErrorMsg.invalidCharacter('~'));
    });
});
